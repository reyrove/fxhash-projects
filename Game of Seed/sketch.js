...
const setup = () => {
  ...
  features.widthdivision =   Math.floor($fx.rand()*100+5)
  features.heightdivision =features.widthdivision+  Math.floor($fx.rand()*10-5)
  features.numOfIt= Math.floor( $fx.rand()*2000+30)
  features.numberOfgroup=Math.floor( $fx.rand()*20+10)

  const readableFeaturesObj = {}
  readableFeaturesObj['Background color'] = backgroundNames [backgroundIndex]
  readableFeaturesObj['Number of Circles'] = features.widthdivision*features.heightdivision
  readableFeaturesObj['Number of itterations'] = features.numOfIt
  readableFeaturesObj['Number of initial groups'] = features.numberOfgroup
...
function generateMatrixWithOnes(rows, cols, numOnesGroups) {
  // Initialize the zero matrix
  const matrix = Array.from({ length: rows }, () => Array(cols).fill(0));

  for (let n = 0; n < numOnesGroups; n++) {
      let placed = false;
      
      while (!placed) {
          // Randomly choose orientation: 0 for horizontal, 1 for vertical
          const orientation = Math.floor($fx.rand() * 2);

          if (orientation === 0) {
              // Horizontal placement
              const row = Math.floor($fx.rand() * rows);
              const col = Math.floor($fx.rand() * (cols - 2)); // Ensure space for 3 ones
              
              if (matrix[row][col] === 0 && matrix[row][col + 1] === 0 && matrix[row][col + 2] === 0) {
                  matrix[row][col] = 1;
                  matrix[row][col + 1] = 1;
                  matrix[row][col + 2] = 1;
                  placed = true;
              ...
              // Vertical placement
              const row = Math.floor($fx.rand() * (rows - 2)); // Ensure space for 3 ones
              const col = Math.floor($fx.rand() * cols);
...
function rules(a, b, c, d, e, f, g, h, i) {
 if (e==0 && [2].includes(a+b+c+d+f+g+h+i)) return 1
 else return 0
}
...
R=0.8
let cells = generateMatrixWithOnes(A2, A1, features.numberOfgroup);
let cells1 = cells;
...
for (let i=0;i<=A1-2;i++){
  for (let j=0;j<=A2-2;j++){
   x=(i+1)*w/A1
   y=(j+1)*h/A2   
   if (cells[j][i]==1 ){
    ...
    ctx.shadowColor=Q;
    ctx.shadowBlur=w/A1;
    ctx.arc(x, y, Math.min(w/(2*A1),h/(2*A2))-Math.min(w/(2*A1),h/(2*A2))/30,0, 2*Math.PI)
    ctx.fill()
    ctx.stroke()
    ctx.closePath()
  }else{
