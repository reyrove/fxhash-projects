...
function drawParametricCurve1(ctx, centerX, centerY, radius, steps,color,Width,color2) {
  ...
  A1=randomgenerat(4,[5,8]);
  A2=randomgenerat(4,[5,8]);

  B1=[0,0,0,0];
  B1[0]=randomgenerat(1,[2,6]);
  B1[1]=randomgenerat(1,[2,6]);
  B1[2]=randomgenerat(1,[2,6]);
  B1[3]=randomgenerat(1,[2,4]);
 
  D1=randomgenerat(4,[0,5])
  

  // Define the parametric equations for a circle
  const parametricX = (T) => centerX + R/B1[0] * Math.sin(T/A1[0]+D1[0])+ R/B1[1] * Math.sin(T/A1[1]+D1[1])+ R/B1[2] * Math.sin(T/A1[2]+D1[2])+ R/B1[3] * Math.sin(T/A1[3]+D1[3]);
  const parametricY = (T) => centerY + R/B1[0] * Math.cos(T/A2[0]+D1[0])+ R/B1[1] * Math.cos(T/A2[1]+D1[1])+ R/B1[2] * Math.cos(T/A2[2]+D1[2])+ R/B1[3] * Math.cos(T/A2[3]+D1[3]);
  
  // Increment angle from 0 to 2*PI with specified steps
  for (let theta = 0; theta <= 2* lcmOfVector(A1.concat(A2)) * Math.PI; theta +=  2*Math.PI / (steps)) {
...
function drawParametricCurve2(ctx, centerX, centerY, radius, steps,color,Width,color2) {
  
  R=radius;

  A1=randomgenerat(4,[1,5]);
  A2=randomgenerat(4,[4,8]);

  B1=[0,0,0,0];
  B1[0]=randomgenerat(1,[3,8]);
  B1[1]=randomgenerat(1,[3,8]);
  B1[2]=randomgenerat(1,[2,6]);
  B1[3]=randomgenerat(1,[2,5]);
 
  C1=randomgenerat(3,[0,2])
  
 
  O=C1[0]/B1[0]+C1[1]/B1[1]+C1[2]/B1[2]+1/B1[3]
  
  ctx.translate(centerX,centerY)

  if(O>=0.5 || O<=0.25){
  ctx.scale(0.8/O,0.8/O)
  }
...
V=A1.concat(A2);
  ...
  // Define the parametric equations for a circle
  const parametricX = (T) =>  R/B1[0] * Math.sin(T/A1[0])+ C1[0]*R/B1[1] * Math.sin(T/A1[1])+ C1[1]*R/B1[2] * Math.sin(T/A1[2])+ C1[2]*R/B1[3] * Math.sin(T/A1[3]);
  const parametricY = (T) =>  R/B1[0] * Math.cos(T/A2[0])+ C1[0]*R/B1[1] * Math.cos(T/A2[1])+ C1[1]*R/B1[2] * Math.cos(T/A2[2]);
  
  // Increment angle from 0 to 2*PI with specified steps
  for (let theta = 0; theta <= 2* lcmOfVector(V) * Math.PI; theta +=  2*Math.PI / (steps*(1+2*(C1[0]+C1[1]+C1[2])))) {
    const x = parametricX(theta);
    const y = parametricY(theta);
    
    // Move to the first point or draw a line segment for subsequent points
    ctx.beginPath();
    ctx.arc(x,y,radius/100*$fx.rand()+radius/(10*lcmOfVector(V)),Math.PI*$fx.rand(),Math.PI+Math.PI*$fx.rand())
...
const drawCanvas = async () => {
  ...
  color31= features.Color3[Math.floor($fx.rand()*features.Color3.length)];
  color32= features.Color3[Math.floor($fx.rand()*features.Color3.length)];
